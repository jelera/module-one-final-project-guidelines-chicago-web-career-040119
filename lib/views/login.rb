...
class LoginView
  ...
  def create_user
    ...
    $user = User.create(
      ...
      age: user[:age],
      address: "#{user[:address][:street].downcase.split.map(&:capitalize).join(' ')}, #{user[:address][:city].downcase.split.map(&:capitalize).join(' ')}, #{user[:address][:state].upcase} #{user[:address][:zip]}",
      email: user[:username],
